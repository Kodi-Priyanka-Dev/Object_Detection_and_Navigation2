"""
YOLOv8n Model Conversion Script: .pt → .tflite
Converts yolov8n.pt to TensorFlow Lite format for mobile deployment
"""
...
def convert_yolov8n_model(model_path, output_dir, img_size=416):
    """
    Convert YOLOv8n .pt model to .tflite format
    
    Args:
        model_path (str): Path to yolov8n.pt model
    ...
    print(f"\n{'='*60}")
    print(f"🚀 YOLOv8n Model Conversion: .pt → .tflite")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        if not tflite_path.startswith(output_dir):
            final_tflite = os.path.join(output_dir, 'yolov8n.tflite')
        ...
        print(f"   3. Ensure yolov8n.pt exists in the project")
        sys.exit(1)
...
if __name__ == "__main__":
    # Configuration - Looks for yolov8n.pt in multiple locations
    possible_paths = [
        "yolov8n.pt",                                    # Current directory
        "../yolov8n.pt",                                 # Parent directory
        "../../yolov8n.pt",                              # Two levels up
        "../Navigation App/yolov8n.pt",                  # Navigation App folder
    ]
    
    BEST_PT_PATH = None
    for path in possible_paths:
        if os.path.exists(path):
            BEST_PT_PATH = os.path.abspath(path)
            print(f"Found yolov8n.pt at: {BEST_PT_PATH}")
            break
    
    if BEST_PT_PATH is None:
        print(f"❌ ERROR: yolov8n.pt not found in any expected location:")
        for path in possible_paths:
    ...
    # Run conversion
    tflite_model = convert_yolov8n_model(
